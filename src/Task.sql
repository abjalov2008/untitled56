...
select owners.first_name, h.house_type from owners join houses h on owners.id = h.owner_id
...
select case when exists(select 1 from houses where price between 1500 and  2000) then 'true' else 'false' end;
...
select houses.id , houses.house_type from houses where id in (5,6,7,8,9,10);
-- 4. Бардык уйлорду, уйдун ээсинин атын, клиенттин атын, агенттин атын чыгар.
select house_type, o.first_name, c.first_name ,name from houses left join rent_info ri on houses.id = ri.house_id
                                                                left join owners o on houses.owner_id = o.id left join customers c on ri.customer_id = c.id
                                                                left join agencies a on ri.agency_id = a.id;
...
select houses.house_type,houses.rating, o.first_name from houses join owners o on o.id = houses.owner_id
order by rating;
...
select agencies.name, a.city , h.house_type , h.address_id from agencies join
                                                                addresses a on agencies.address_id = a.id join houses h on a.id = h.address_id;
-- 9. Уйлордун арасынан мебели бар уйлорду, уйдун ээсин жана уйдун адрессин чыгар.
select houses.house_type, houses.furniture , o.first_name, houses.address_id from houses join owners o on o.id = houses.owner_id
                                                                                         join addresses a on a.id = houses.address_id where furniture is  not null;
-- 10.Кленти жок уйлордун баарын жана анын адрессин жана ал уйлор кайсыл агентсвога тийешелуу экенин чыгар.
select houses.house_type, houses.address_id, a2.name from houses join addresses a on a.id = houses.address_id
                                                                 join agencies a2 on a.id = a2.address_id join rent_info ri on houses.id = ri.house_id
where customer_id is null;
-- 11.Клиенттердин улутуна карап, улутуну жана ал улуутта канча клиент жашайт санын чыгар.
select customers.nationality , count(customers.id) from customers group by nationality;
...
select c.first_name, houses.house_type from houses join rent_info ri on houses.id = ri.house_id
                                                   join customers c on c.id = ri.customer_id where ri.customer_id is null and ri.house_id is null;
...
select o.first_name, c.first_name from houses join rent_info ri on houses.id = ri.house_id
                                              join owners o on o.id = houses.owner_id join customers c on c.id = ri.customer_id
where house_type like 'A%';
SELECT
...
FROM
    houses
        JOIN
    rent_info ri ON houses.id = ri.house_id
        JOIN
    owners o ON o.id = houses.owner_id
        JOIN
    customers c ON c.id = ri.customer_id
WHERE
    houses.house_type LIKE 'A%';
-- 16.Эн уйу коп owner_ди жана анын уйлорунун санын чыгар.
select owners.first_name , count(h.house_type) from owners join houses h on owners.id = h.owner_id
group by owners.first_name order by first_name desc limit 1;
-- 17.Улуту Kyrgyzstan уй-булолу customerлерди чыгарыныз.
select customers.first_name , customers.nationality from customers where nationality= 'Kyrgyz';
-- 18.Эн коп болмолуу уйду жана анын адресин ал уй кайсыл ownerге таандык ошону чыгарыныз.
select houses.house_type, houses.room, houses.address_id, o.first_name from houses join owners o on o.id = houses.owner_id
                                                                                   join addresses a on a.id = houses.address_id where room= (select max(room) from houses);
-- 19.Бишкекте жайгашкан уйлорду жана клиентерин кошо чыгарыныз.
select houses.house_type, c.first_name from houses join rent_info ri on houses.id = ri.house_id
                                                   join customers c on c.id = ri.customer_id join addresses a on a.id = houses.address_id
where city = 'Bishkek';
-- 20.Жендерине карап группировка кылыныз.
select gender, count(*) from customers group by gender;
-- 21.Эн коп моонотко ижарага алынган уйду чыгарыныз.
select houses.house_type, greatest(ri.check_out-ri.check_in) as max_rent_day from houses join rent_info ri
                                                                                              on houses.id = ri.house_id order by max_rent_day desc limit 1;
-- 22.Эн кымбат уйду жана анын ээсин чыгарыныз.
select houses.house_type, o.first_name,price from houses join owners o on o.id = houses.owner_id
where price= (select max(price)from houses);
-- 23.Бир региондо жайгашкан баардык агентстволорду чыгарыныз
select agencies.name , agencies.address_id from agencies join addresses a on a.id = agencies.address_id
where region ='BR-AM';
-- 24.Рейтинг боюнча эн популярдуу 5 уйду чыгар.
select houses.house_type, houses.rating from houses order by rating desc limit 5;
-- 25.Орто жаштагы owner_ди , анын уйун , уйдун адрессин чыгар.
select owners.first_name, extract(year from age(current_date, owners.date_of_birth)) as age, houses.house_type, houses.address_id
from owners join houses on owners.id = houses.owner_id join addresses on houses.address_id = addresses.id
where extract(year from age(current_date, owners.date_of_birth))=40;
